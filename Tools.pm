...
but unlike L<Tools::get_unchecked_large_number>, it plays it back to the user for confirmation.
...
=head2 get_unchecked_small_number
...
When the user is prompted to speak, this method records the audio to the
filesystem.
...
Checks the database for the list of allowed actions to determine what action 
...
Returns the current user's username/nickname
